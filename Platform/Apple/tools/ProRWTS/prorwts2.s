...
!if (enable_write + enable_seek + allow_multi + rwts_mode) > 0 {
                ...
                !set last_zp = $D       ;highest address to save if swap_zp enabled (max 127 entries later)
...
!if mem_swap = 0 {
  ...
  !if allow_trees = 1 {
                ...
                !set last_zp = $1b      ;highest address to save if swap_zp enabled (max 127 entries later)
    !if always_trees = 0 {
  ...
  !if aligned_read = 0 {
                ...
                !set last_zp = $1a      ;highest address to save if swap_zp enabled (max 127 entries later)
  } ;aligned_read = 0
} ;mem_swap = 0
...
!if enable_floppy = 1 {
                ...
                !set last_zp = $1f      ;highest address to save if swap_zp enabled (max 127 entries later)
} ;enable_floppy = 1
